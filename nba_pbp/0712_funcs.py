...
def get_gamepbp(game_id, game_hvv):
    ...
    if len(pbp2_df['PERIOD'].unique()) == 4:
        pbp2_df.insert(7, 'SECONDS_LEFT', (4 - pbp2_df['PERIOD']) * 12 * 60 + pd.Series(
            ...
            map(int, pbp2_df['PCTIMESTRING'].str.partition(':')[2])))
    ...
    elif len(pbp2_df['PERIOD'].unique()) == 6:
        pbp2_df.insert(7, 'SECONDS_LEFT', (4 - pbp2_df['PERIOD']) * 12 * 60 + pd.Series(
            map(int, pbp2_df['PCTIMESTRING'].str.partition(':')[0])) * 60 + pd.Series(
            map(int, pbp2_df['PCTIMESTRING'].str.partition(':')[2])) + 600)
        pbp2_df.loc[pbp2_df['PERIOD'] == 5, 'SECONDS_LEFT'] += 420
        pbp2_df.loc[pbp2_df['PERIOD'] == 6, 'SECONDS_LEFT'] += 840
...
"""new functions which, fingers crossed, seem to work with testing"""
...
def get_subs(pbp2_df):
    ...
    for idx in pbp2_df[(pbp2_df['EVENTMSGTYPE'] == 8) | (pbp2_df['EVENTMSGTYPE'] == 12)].index:
        ...
        if idx <= last_idx:
            ...
            last_idx = pbp2_df[
                (pbp2_df['PERIOD'] == sub_period) & (pbp2_df['PCTIMESTRING'] == sub_timestring)].index.max()

        stint_end_idxs.append(last_idx)

    # merging the lists of indexes for quarterchange, sub, last index of same timestring as sub (aka subsequent FT)
    ...
    #copying the original get_subs to include the rows of subsequent FTs into the subs_df
    subs_df = pbp2_df.iloc[resulting_list]
    subs_df.insert(3, 'subs_stint_bool', subs_df.SECONDS_LEFT == subs_df.SECONDS_LEFT.shift())
...
def get_gamestints():
    ...
    for sub_stint in subs_df['subs_stint'].unique():
        ...
        if (subs_df[subs_df['subs_stint'] == sub_stint]['EVENTMSGTYPE'] == 12).any():
    ...
    if len(pbp2_df['PERIOD'].unique()) == 4:
        ...
        game_stints_df.loc[len(game_stints_df) - 1, ['end_score']] = pbp2_df[(pbp2_df['EVENTMSGTYPE'] == 13) & (pbp2_df['PERIOD'] == 6)]['SCORE'].item()
    ...
    if len(pbp2_df['PERIOD'].unique()) == 4:
        ...
        game_stints_df.loc[len(game_stints_df) - 1, ['end_eventnum']] = pbp2_df[(pbp2_df['EVENTMSGTYPE'] == 13) & (pbp2_df['PERIOD'] == 5)]['EVENTNUM'].item()
    elif len(pbp2_df['PERIOD'].unique()) == 6:
        game_stints_df.loc[len(game_stints_df) - 1, ['end_eventnum']] = pbp2_df[(pbp2_df['EVENTMSGTYPE'] == 13) & (pbp2_df['PERIOD'] == 6)]['EVENTNUM'].item()
